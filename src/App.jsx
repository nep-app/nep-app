...
import * as Icons from './components/Icons';
        ...
        function HarmReductionTracker() {
            ...
            const getEmotionalPatterns = () => {
                if (wellbeingLogs.length === 0) return null;

                // Count emotions on high consumption days vs low consumption days
                const emotionsByConsumption = { high: {}, low: {} };

                wellbeingLogs.forEach(w => {
                    const dayConsumptions = consumptions.filter(c => c.date === w.date).length;
                    const isHigh = dayConsumptions > 3; // More than 3 consumptions = high
                    const category = isHigh ? 'high' : 'low';

                    if (w.emotions && w.emotions.length > 0) {
                        w.emotions.forEach(emotion => {
                            emotionsByConsumption[category][emotion] = (emotionsByConsumption[category][emotion] || 0) + 1;
                        });
                    }
                });

                // Find emotions that appear significantly more on high consumption days
                const riskEmotions = [];
                const protectiveEmotions = [];

                Object.keys(emotionsByConsumption.high).forEach(emotion => {
                    const highCount = emotionsByConsumption.high[emotion] || 0;
                    const lowCount = emotionsByConsumption.low[emotion] || 0;
                    const total = highCount + lowCount;

                    if (total >= 3) { // Only consider emotions that appear at least 3 times
                        const highRatio = highCount / (highCount + lowCount);
                        if (highRatio > 0.65) {
                            riskEmotions.push({ emotion, highCount, lowCount, ratio: highRatio });
                        } else if (highRatio < 0.35) {
                            protectiveEmotions.push({ emotion, highCount, lowCount, ratio: highRatio });
                        }
                    }
                });

                return {
                    riskEmotions: riskEmotions.sort((a, b) => b.ratio - a.ratio),
                    protectiveEmotions: protectiveEmotions.sort((a, b) => a.ratio - b.ratio),
                    totalEmotionsTracked: new Set([...Object.keys(emotionsByConsumption.high), ...Object.keys(emotionsByConsumption.low)]).size
                };
            };

            // Analyze sentiment in reflections and notes
            const getSentimentAnalysis = () => {
                const allReflections = reflections.filter(r => r.answer && r.answer.trim().length > 0);
                const allWellbeingNotes = wellbeingLogs.filter(w => w.notes && w.notes.trim().length > 0);
                const allConsumptionNotes = consumptions.filter(c => c.notes && c.notes.trim().length > 0);
                const allDailyNotes = dailyLogs.filter(d => d.notes && d.notes.trim().length > 0);
                const allCycleNotes = cycles.filter(cy => cy.notes && cy.notes.trim().length > 0);

                const totalTexts = allReflections.length + allWellbeingNotes.length + allConsumptionNotes.length + allDailyNotes.length + allCycleNotes.length;
                if (totalTexts === 0) return null;

                // Analyze sentiment per entry with consumption data
                const sentimentByDay = {};

                // DBT Reflections
                allReflections.forEach(r => {
                    const sentiment = analyzeSentiment(r.answer);
                    const dayConsumptions = consumptions.filter(c => c.date === r.date).length;

                    if (!sentimentByDay[r.date]) sentimentByDay[r.date] = { sentiments: [], consumptions: dayConsumptions };
                    sentimentByDay[r.date].sentiments.push({ ...sentiment, type: 'reflection', text: r.answer.substring(0, 100) });
                });

                // Wellbeing Notes
                allWellbeingNotes.forEach(w => {
                    const sentiment = analyzeSentiment(w.notes);
                    const dayConsumptions = consumptions.filter(c => c.date === w.date).length;

                    if (!sentimentByDay[w.date]) sentimentByDay[w.date] = { sentiments: [], consumptions: dayConsumptions };
                    sentimentByDay[w.date].sentiments.push({ ...sentiment, type: 'wellbeing', text: w.notes.substring(0, 100) });
                });

                // Consumption Notes
                allConsumptionNotes.forEach(c => {
                    const sentiment = analyzeSentiment(c.notes);
                    const dayConsumptions = consumptions.filter(cons => cons.date === c.date).length;

                    if (!sentimentByDay[c.date]) sentimentByDay[c.date] = { sentiments: [], consumptions: dayConsumptions };
                    sentimentByDay[c.date].sentiments.push({ ...sentiment, type: 'consumption', text: c.notes.substring(0, 100) });
                });

                // Daily Log Notes
                allDailyNotes.forEach(d => {
                    const dayConsumptions = consumptions.filter(c => c.date === d.date).length;

                    if (!sentimentByDay[d.date]) sentimentByDay[d.date] = { sentiments: [], consumptions: dayConsumptions };
                    sentimentByDay[d.date].sentiments.push({ ...analyzeSentiment(d.notes), type: 'daily', text: d.notes.substring(0, 100) });
                });

                // Cycle Notes
                allCycleNotes.forEach(cy => {
                    const date = safeToISODate(cy.timestamp);
                    if (!date) return;
                    const dayConsumptions = consumptions.filter(c => c.date === date).length;

                    if (!sentimentByDay[date]) sentimentByDay[date] = { sentiments: [], consumptions: dayConsumptions };
                    sentimentByDay[date].sentiments.push({ ...analyzeSentiment(cy.notes), type: 'cycle', text: cy.notes.substring(0, 100) });
                });

                // Calculate average sentiment by consumption level
                const highConsumptionDays = Object.entries(sentimentByDay).filter(([_, data]) => data.consumptions > 3);
                const lowConsumptionDays = Object.entries(sentimentByDay).filter(([_, data]) => data.consumptions <= 3);

                const getAvgSentiment = (days) => {
                    if (days.length === 0) return 0;
                    const allSentiments = days.flatMap(([_, data]) => data.sentiments);
                    return allSentiments.reduce((sum, s) => sum + s.score, 0) / allSentiments.length;
                };

                return {
                    avgSentimentHighConsumption: getAvgSentiment(highConsumptionDays),
                    avgSentimentLowConsumption: getAvgSentiment(lowConsumptionDays),
                    totalReflections: allReflections.length,
                    totalWellbeingNotes: allWellbeingNotes.length,
                    totalConsumptionNotes: allConsumptionNotes.length,
                    totalDailyNotes: allDailyNotes.length,
                    totalCycleNotes: allCycleNotes.length,
                    totalTexts: totalTexts,
                    daysAnalyzed: Object.keys(sentimentByDay).length
                };
            };
