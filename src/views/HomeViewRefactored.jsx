...
export function HomeViewRefactored({
  ...
  const alerts = useHomeAlerts(goals, metrics, cycles, dailyLogs);
  const stats = useHomeStats(metrics, cycles, currentCycleCount, badges);
      ...
      {stats.timeSince && (
        <div className="flex justify-center mb-4">
          <InfoBadge
            ...
            value={`${stats.timeSince.value}${stats.timeSince.unit}`}
            subValue={stats.timeSince.subValue}
            subUnit={stats.timeSince.subUnit}
            isPositive={stats.timeSince.isLong}
            darkMode={darkMode}
          />
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className={(darkMode ? 'bg-gradient-to-br from-purple-900/20 to-purple-800/10' : 'bg-gradient-to-br from-purple-50 to-purple-100/50') + ' rounded-xl p-3'}>
          ...
          <div className="flex items-baseline gap-1">
            <span className={'text-2xl font-black ' + (darkMode ? 'text-purple-300' : 'text-purple-600')}>{stats.currentCycleCount}</span>
        ...
        <div className={(darkMode ? 'bg-gradient-to-br from-pink-900/20 to-pink-800/10' : 'bg-gradient-to-br from-pink-50 to-pink-100/50') + ' rounded-xl p-3'}>
          ...
          <div className="flex items-baseline gap-1">
            <span className={'text-2xl font-black ' + (darkMode ? 'text-pink-300' : 'text-pink-600')}>{stats.last7.avgTimes}</span>
            <span className={(darkMode ? 'text-pink-400' : 'text-pink-500') + ' text-sm font-medium'}>x</span>
          </div>
          <div className={(darkMode ? 'text-pink-500' : 'text-pink-400') + ' text-xs font-medium mt-0.5'}>{stats.last7.avgMg}mg/dia</div>
        </div>
      </div>
